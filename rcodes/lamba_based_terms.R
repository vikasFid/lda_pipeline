...
#relTerms = c()
added_words = c()
#add numbers for topics of interest, ALL for all topics
selected_topics = c(1,8,48)
if(selected_topics[1] == 'ALL'){
num_selected_topics = num_topics
rel_topic_list = c(1:num_topics)
}else{
num_selected_topics = length(selected_topics)
rel_topic_list = selected_topics
}
...
for (i in rel_topic_list){
	...
	for(term in myAPdata$relevance[,1]){
		...
		if(!term %in% added_words){
			...
			#fetch the topics that satisfy the freq cutoff >= 0.5(optimal threshold through experimentation)
			...
			if (!i == top_topic_set[1] || nrow(word_topic_dis) > num_topics/2){
				relevant_term_flag = 0	
			}
...
write.csv(output,file = 'outputFiles/lambda_topic_terms.csv',
          row.names = F, quote =  F)
