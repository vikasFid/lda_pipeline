...
#inputFile = args[1]
txt_input = read.csv('SCRIPT_NLP_CONTEXT.csv',
                     header = TRUE,sep = ',',colClasses = "character")


txt_data = as.character(txt_input[,7])
...
save(modelTrain,file = "outputFiles/ldaModel.RData")
...
save(json, file = "outputFiles/LDAVis_JSON.RData")
save(myAPdata, file = "outputFiles/myAPdata.RData")

#load(file = "C:/Users/a592407/Desktop/LDA_RDATA/LDAVis_REDEEM_ALL.RData")
serVis(json,out.dir="lda_viz", open.browser = F)
...
write.csv(topicProbabilities,file="outputFiles/TOPIC_PROB.csv",
           row.names = F)
...
doc_df = data.frame(interaction_id = txt_input[doc_index,1],
                     ...
                     cleanPhrase = txt_input[doc_index,6],
		     mergedPhrase = txt_input[doc_index,5],
                     topic = docTopic[,2])

write.csv(doc_df,file = "outputFiles/DOC_TOPIC.csv",row.names = F)
...
write.csv(groupedData[order(-groupedData$topicProportion),],
          'outputFiles/DOC_TOPIC_PROP.csv',quote = F,
          row.names = F)
