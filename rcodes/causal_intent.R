...
for (i in selected_topics){
	...
	for(term in myAPdata$relevance[1:35,1]){
		relevant_term_flag = 1
		#print(term)
  		if (!term%in%checked_words){

			#we only need to see a term once as we are checking for the relevance across topics
			checked_words_counter = length(checked_words)
			checked_words[checked_words_counter + 1] = term
			...
			#print(paste("Top topic set::",top_topic_set))
			#now check if the top K topics are the selected topics in any order
			#print(top_topic_set)
			for(k in selected_topics){
				#get top K topics for the term:
				# very strict first, term should have the K topics as top_topics


				#improve this logic, how to determine better term - topic mapping
				
				

				if ((nrow(word_topic_dis) > 10) || (!k %in% top_topic_set)){
...
write.csv(relTerms,file = 'outputFiles/TOPIC_LIST_TERMS.csv',
          row.names = F, quote =  F)
