...
def getTopicTerms():
    df_topic_words = pd.read_table("adhocOutput/topic_terms.csv", sep=',', header=(0))
...
def getTopicContext(context,topic_string):
    ...
    df = pd.read_table("adhocOutput/DOC_TOPIC.csv", sep=',', header=(0),dtype={'nice_interaction_id': 'str'})	
    #df = df.dropna()
    df_topic_prob = pd.read_csv("adhocOutput/TOPIC_PROB.csv", sep = ',')

    df_clean_trans = rc.DataFrame(columns=['doc_id','doc_topics','topic_words', 'phrase','origPhrase'])
    ...
    for index,row in df.iterrows():	
		...
		interaction_id = row['doc_id']
                mergedPhrase = row['mergedPhrase']
		chunkedPhrase = row['chunkedPhrase']
		doc_topics = []
		...
		for i,column in enumerate(selected_topic_words.columns):
			...
			if topic_presence == 1:
				...
				if '-' not in topic_terms_sen and len(doc_topic_words) == 1:
					topic_presence = 0;
				else:

					doc_topics.append(i+1)
                			#print(mergedPhrase)
    ...
    df_clean_trans.to_csv('adhocOutput/lambda_based_topic.csv',index = False)
