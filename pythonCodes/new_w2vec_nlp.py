...
def extract_useful_terms(interaction_id,nice_interaction_id,psent,customer_id,call_date,gensimPhrase):
    ...
    with open('SCRIPT_NLP_CONTEXT.csv','a') as f:
        ...
        fline = str(interaction_id) + ',' + str(nice_interaction_id) + ',' + str(customer_id) + ',' + str(call_date) + ',' + merged_phrase + ',' + gensimPhrase
        f.write(fline+'\n')
...
def chunkProcessedTrans(df_clean_trans):
    ...
    for index,row in df_clean_trans.iterrows():
        ...
        extract_useful_terms(interaction_id,nice_interaction_id,adjResult,customer_id,call_date,cleanTrans)
