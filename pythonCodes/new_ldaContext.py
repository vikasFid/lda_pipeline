...
def getTopicTerms(topic,numTopics):
    ...
    topic_words = []
    for w in words:
        if word_dict[w] < 4 and w not in topic_words:
...
def getTopicContext(population,topicNum,numTopics):
    ...
    df_clean_trans = rc.DataFrame(columns=['doc_topic_score','interaction_id','nice_interaction_id','customer_id','call_date', 'phrase','origPhrase'])
	...
	topic_prob = row[0]
	if topic_prob >= topic_prob_cutoff:
		...
		doc_topic_score = topic_prob
        	mergedPhrase = str(mergedPhrase)
        	#runningWindowSpan(interaction_id,nice_interaction_id,customer_id,call_date,trans,topic_words,12,0.25,population,topicNum)
        	#chunkList = [chunk for chunk in phrase.split('|')]
