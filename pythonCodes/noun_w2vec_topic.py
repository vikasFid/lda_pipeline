...
def extract_useful_terms(interaction_id, nice_interaction_id, psent, customer_id, call_date, gensimPhrase, nouns):
    ...
    with open('SCRIPT_NLP_CONTEXT.csv', 'a') as f:
        ...
        fline = str(interaction_id) + ',' + str(nice_interaction_id) + ',' + str(customer_id) + ',' + str(
            call_date) + ',' + merged_phrase + ',' + gensimPhrase + ',' + nouns
        f.write(fline + '\n')
...
def parseTranscripts():
    ...
    if os.path.isfile('SCRIPT_NLP_CONTEXT.csv'):
        ...
        with open('SCRIPT_NLP_CONTEXT.csv', 'a') as f:
            f.write(
                'interaction_id,nice_interaction_id,customer_id,call_date,cleanPhrase,mergedPhrase,parsedNouns' + '\n')

    # cleaning transcripts after collecting lemmas using spacy lemmatizer
    df_clean_trans = rc.DataFrame(
        columns=['interaction_id', 'nice_interaction_id', 'customer_id', 'call_date', 'phrase', 'nouns'])
...
def phraseBuilder(df_sentences):
    ...
    for index, row in df_sentences.iterrows():
        sentence = [word for word in word_tokenize(cleanTranscript(row['transcript']))]
    ...
    df_sentences = df_sentences.drop('transcript', 1)
