...
def getTopicTerms():
    df_topic_words = pd.read_table("custom_string_search.csv", sep=',', header=(0))
...
def getTopicContext(population,topic_string):
    ...
    df_selected = pd.read_table("outputFiles/lambda_based_topic.csv", sep=',', header=(0),dtype={'nice_interaction_id': 'str'})
    selected_interaction_id_list = df_selected['interaction_id'].tolist()
    #df = df.dropna()
    ...
    for index,row in df.iterrows():	
		...
		if interaction_id in selected_interaction_id_list:
			continue
    ...
    df_clean_trans.to_csv('custom_search_output.csv')
