...
def extract_useful_terms(interaction_id,nice_interaction_id,psent,customer_id,call_date,gensimPhrase,nouns):
    ...
    with open('SCRIPT_NLP_CONTEXT.csv','a') as f:
        ...
        fline = str(interaction_id) + ',' + str(nice_interaction_id) + ',' + str(customer_id) + ',' + str(call_date) + ',' + merged_phrase + ',' + gensimPhrase + ',' + nouns
        f.write(fline+'\n')
...
def chunkProcessedTrans(df_clean_trans):
    ...
    for index,row in df_clean_trans.iterrows():
        ...
        extract_useful_terms(interaction_id,nice_interaction_id,adjResult,customer_id,call_date,cleanTrans,nouns)
...
def parseTranscripts():
    ...
    df = pd.read_table("outTrans.tsv", sep='\t', header=(0),nrows = 10000)
	...
	with open('SCRIPT_NLP_CONTEXT.csv', 'a') as f:
            f.write('interaction_id,nice_interaction_id,customer_id,call_date,cleanPhrase,mergedPhrase,parsedNouns'+'\n')


    #cleaning transcripts after collecting lemmas using spacy lemmatizer
    df_clean_trans = rc.DataFrame(columns=['interaction_id','nice_interaction_id','customer_id','call_date', 'phrase','nouns'])
    ...
    for index, row in df.iterrows():
        ...
        posTaggedWordPhrase = ' '.join(w.lemma_ + '_' + wordPosTag(w) for w in spacyPhrase if w.lemma_ not in spacy.en.STOP_WORDS and wordPosTag(w) != '')
        nounTaggedWords = ' '.join(w.lemma_ for w in spacyPhrase if (w.pos_ == 'NOUN' or w.pos_ == 'PROPN' or w.pos_ == 'NUM') and w.lemma_ not in spacy.en.STOP_WORDS and wordPosTag(w) != '')



	if interaction_id not in spacyPhraseDict:
		spacyPhraseDict[interaction_id] = posTaggedWordPhrase
        	nounDict[interaction_id] = nounTaggedWords
    ...
    for index, row in df.iterrows():
        ...
        nounPhrase = nounDict[interaction_id]
        rowNum += 1
        print('Processing Row : %s' % rowNum)
...
def nlpOutput(df_phrased):
	for index,row in df_phrased.iterrows():
		...
		customer_id = row['customer_id']
		call_date = row['call_date']
		gensimPhrase = row['finalTrans']

		with open('SCRIPT_NLP_CONTEXT.csv','a') as f:
			fline = str(interaction_id) + ',' + str(nice_interaction_id) + ',' + str(customer_id) + ',' + str(call_date) + ',' + gensimPhrase
        		f.write(fline+'\n')
